import { Activity, ActivitySquare, Calculator, CalendarDays, Clock, Droplet, Dumbbell, Flame, Globe, MapPin, Moon, Sun, Target, User, X } from 'lucide-react';
...
function App() {
  ...
  const t = {
    tr: {
      ...
      daysTitle: "Haftalık Gün", dayStr: "Gün", time30: "30 Dakika (Hızlı)", time45: "45 Dakika", time60: "1 Saat (İdeal)", time90: "1.5 Saat (Yoğun)",
      ...
      howTo: "Nasıl Yapılır?", coachTip: "Antrenör Notu", close: "Kapat",
    ...
    en: {
      ...
      daysTitle: "Days/Week", dayStr: "Day", time30: "30 Mins (Quick)", time45: "45 Mins", time60: "1 Hour (Ideal)", time90: "1.5 Hours (Intense)",
      gym: "Gym (Full Equip)", home: "Home (Basic Equip)", focusNone: "Balanced (Full Body)", chest: "Chest", arms: "Arms", legs: "Legs",
      ...
      howTo: "How to perform?", coachTip: "Coach's Tip", close: "Close",
  ...
  const generatePremiumRoutine = (days, time, location, focus) => {
    ...
    const pools = {
      ...
      Legs: isHome ? ["Air Squat", "Lunges", "Glute Bridge", "Calf Raise", "Jump Squat"] : ["Squat", "Leg Press", "Romanian Deadlift", "Leg Extension", "Calf Raise"],
    ...
    let plan = [];
    if (days === 3) {
      plan = [{ type: "FullBody" }, { isRest: true }, { type: "FullBody" }, { isRest: true }, { type: "FullBody" }, { isRest: true }, { isRest: true }];
    } else if (days === 4) {
      plan = [{ type: "Upper" }, { type: "Lower" }, { isRest: true }, { type: "Upper" }, { type: "Lower" }, { isRest: true }, { isRest: true }];
    } else if (days === 5) {
      plan = [{ type: "Push" }, { type: "Pull" }, { type: "Legs" }, { isRest: true }, { type: "Upper" }, { type: "Lower" }, { isRest: true }];
    } else {
      plan = [{ type: "Push" }, { type: "Pull" }, { type: "Legs" }, { type: "Push" }, { type: "Pull" }, { type: "Legs" }, { isRest: true }];
    }
  ...
  // Dinamik Temalar (Tam Profesyonel Renkler)
          ...
          <div className={`xl:col-span-4 rounded-3xl p-7 border ${card}`}>
            ...
            <form onSubmit={calculateAdvanced} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.gender}</label><select name="gender" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 transition ${inputBg}`}><option value="male">{curr.male}</option><option value="female">{curr.female}</option></select></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.age}</label><input type="number" name="age" required onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 transition ${inputBg}`} /></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.height}</label><input type="number" name="height" required onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 transition ${inputBg}`} /></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.weight}</label><input type="number" name="weight" required onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 transition ${inputBg}`} /></div>
              </div>

              <div className="space-y-4 pt-4 border-t border-slate-500/20">
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.goal}</label><select name="goal" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 ${inputBg}`}><option value="maintain">{curr.goalMaintain}</option><option value="lose">{curr.goalLose}</option><option value="gain">{curr.goalGain}</option></select></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5">{curr.activityTitle}</label><select name="activityLevel" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-500 ${inputBg}`}><option value="1.2">{curr.act1}</option><option value="1.375">{curr.act2}</option><option value="1.55">{curr.act3}</option><option value="1.725">{curr.act4}</option></select></div>
              </div>

              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-500/20">
                <div><label className="block text-xs font-bold opacity-70 mb-1.5 flex items-center gap-1"><MapPin size={12} /> {curr.location}</label><select name="location" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none ${inputBg}`}><option value="gym">{curr.gym}</option><option value="home">{curr.home}</option></select></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5 flex items-center gap-1"><Clock size={12} /> {curr.time}</label><select name="workoutTime" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none ${inputBg}`}><option value="30">{curr.time30}</option><option value="45">{curr.time45}</option><option value="60">{curr.time60}</option><option value="90">{curr.time90}</option></select></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5 flex items-center gap-1"><Target size={12} /> {curr.focus}</label><select name="focusMuscle" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none ${inputBg}`}><option value="none">{curr.focusNone}</option><option value="chest">{curr.chest}</option><option value="arms">{curr.arms}</option><option value="legs">{curr.legs}</option></select></div>
                <div><label className="block text-xs font-bold opacity-70 mb-1.5 flex items-center gap-1"><CalendarDays size={12} /> {curr.daysTitle}</label><select name="daysPerWeek" onChange={handleInputChange} className={`w-full rounded-xl p-3 outline-none ${inputBg}`}><option value="3">3 {curr.dayStr}</option><option value="4">4 {curr.dayStr}</option><option value="5">5 {curr.dayStr}</option><option value="6">6 {curr.dayStr}</option></select></div>
              </div>
              <button type="submit" className="w-full mt-6 bg-gradient-to-r from-indigo-600 to-blue-500 hover:from-indigo-500 hover:to-blue-400 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-indigo-500/30 flex justify-center items-center gap-2"><Calculator size={20} /> {curr.calcBtn}</button>
            </form>
          </div>
          ...
          <div className="xl:col-span-8 space-y-6">
            ...
            {results ? (
              <div className={`border rounded-3xl p-7 ${card}`}>
                <h3 className="text-2xl font-black mb-6 flex items-center gap-2"><Flame className="text-orange-500" /> {curr.report}</h3>

                <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                  {/* BMI Alanı YENİ EKLENDİ */}
                  <div className={`col-span-2 md:col-span-2 p-5 rounded-2xl border-b-4 ${results.bmiCat === 'normal' ? 'border-b-emerald-500' : results.bmiCat === 'over' ? 'border-b-orange-500' : 'border-b-red-500'} ${inputBg}`}>
                    <div className="text-sm opacity-70 mb-1">{curr.bmi}</div>
                    <div className="text-3xl font-black">{results.bmi}</div>
                    <div className="text-xs font-bold mt-1 opacity-80">{curr.bmiStatus[results.bmiCat]}</div>
                  </div>
              ...
              <div className={`border border-dashed rounded-3xl p-12 text-center h-48 flex flex-col items-center justify-center ${card}`}>
                <h3 className="text-xl font-bold opacity-80 mb-2">{curr.wait}</h3>
                <p className="opacity-50 text-sm">{curr.waitSub}</p>
              </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {workoutPlan.map((item, index) => (
                    <div key={index} className={`p-5 rounded-2xl border transition-all ${inputBg} ${item.isRest ? 'opacity-70' : 'hover:border-indigo-500/50 hover:shadow-lg'}`}>
                      ...
                      {item.isRest ? (
                        <p className="text-sm opacity-60 flex items-center gap-2"><Moon size={16} /> {curr.restDesc}</p>
                      ) : (
                          ...
                          {item.exercises.map((ex, idx) => (
                            <li key={idx} onClick={() => openExerciseInfo(ex.name)} className="flex items-center justify-between p-3 bg-slate-500/5 rounded-xl hover:bg-indigo-500/10 cursor-pointer group transition">
                              <div className="flex flex-col">
                                <span className={`font-semibold text-sm ${ex.isFocus ? 'text-orange-400' : ''}`}>{ex.name}</span>
                                {ex.isFocus && <span className="text-[10px] text-orange-500/80 uppercase mt-0.5">Focus Muscle</span>}
                              </div>
